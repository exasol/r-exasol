...
Enables R users to send SQL queries to an Exasol database and retrieve results through DBI-compliant methods, supporting both immediate full-result retrieval and incremental row fetching.

## Background

Query execution operates over an active EXAConnection. SELECT queries use high-speed data transfer by default via `exa.readData`. Non-SELECT statements are executed directly via RODBC. Result sets from `dbSendQuery` are stored in temporary tables for incremental fetching.
...
* *GIVEN* an active EXAConnection exists
* *WHEN* the user calls `dbGetQuery(conn, "CREATE TABLE ...")`
* *THEN* the statement SHALL be executed via RODBC
