...
EXAConnectionLog <- function (con, view = TRUE) {
  ...
  logItems <- function(l) {
    time <- regmatches(l,gregexpr("^\\d\\d:\\d\\d:\\d\\d.\\d\\d\\d",l, perl=TRUE))
    time <- substr(time,0,nchar(time))
    num <- substr(regmatches(l,gregexpr("^.{12}\\t\\d+?\\t",l, perl=TRUE)),14,18)
    text <- sub("^.{12}\\t\\d+?\\t", "",l)
    data.frame(time=time,num=num, text=text)
  }

  lstr <- regmatches(con@init_connection_string,gregexpr("EXALOGFILE=\\S+?;",con@init_connection_string,perl=TRUE))[[1]]
